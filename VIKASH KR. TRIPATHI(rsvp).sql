...
FROM 
    Movie m
LEFT JOIN 
    Genre g ON m.genre_id = g.genre_id  -- Use LEFT JOIN to include movies without genres
GROUP BY 
    g.genre
ORDER BY 
...
FROM 
    Movie m
JOIN 
    Genre g ON m.genre_id = g.genre_id  -- Adjust the join condition based on your schema
WHERE 
    m.title LIKE 'The%'  -- Filter for titles that start with 'The'
GROUP BY 
    g.genre  -- Group by genre only
HAVING 
    AVG(m.rating) > 8  -- Filter for average rating greater than 8
ORDER BY 
    g.genre;  -- Optional: Order by genre
...
WITH ActressRatings AS (
    SELECT 
        ...
        COUNT(m.id) AS movie_count,
        SUM(r.total_votes) AS total_votes,  -- Sum of total votes for the actress
    ...
    GROUP BY 
        nm.name
    HAVING 
        COUNT(m.id) >= 3  -- Count the number of movies, not the country
)

SELECT 
    actress_name,
    movie_count,
    total_votes,  -- Include total_votes in the final output
    actress_avg_rating,
...
WITH ActressRatings AS (
    SELECT 
        n.name AS actress_name,  -- Explicitly aliasing the name as actress_name
...
SELECT 
    actress_name,  -- Use the alias for actress name here
    total_votes,
    movie_count,
    avg_rating,
